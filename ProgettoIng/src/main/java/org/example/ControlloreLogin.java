...
import org.example.classiEffettive.Diabetologo;
import org.example.classiEffettive.GestoreFile;
import org.example.classiEffettive.Paziente;
import org.example.classiEffettive.Persona;
...
public class ControlloreLogin {
    ...
    private void FaiLogin(ActionEvent event) {
        ...
        try {
            // Cerca l'utente nei rispettivi file JSON (restituisce Diabetologo, Paziente oppure null)
            Persona utente = GestoreFile.cercaPerCredenziali(username, password);
            ...
            if (utente instanceof Diabetologo) {
                ...
                controller.SettaDatiMedico((Diabetologo) utente); // Passa direttamente l'oggetto medico
                ...
                controller.settaDatiPaziente((Paziente) utente); // Passa direttamente l'oggetto paziente
            } else {
                mostraMessErrore("Ruolo utente non riconosciuto.");
                return;
            }

            // Mostra la nuova dashboard e chiude la finestra di login
