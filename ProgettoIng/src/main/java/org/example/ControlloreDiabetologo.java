...
public class ControlloreDiabetologo {
    ...
    //cerca il paziente usando il nome e il cognome nella textBox
    @FXML
    private void cercaPaziente(ActionEvent evento) throws IOException {
        ...
        if (paziente != null) {
            ...
            LocalDateTime limiteSettimana = LocalDateTime.now().minusDays(7);
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM HH:mm");

            XYChart.Series<String, Number> serie = new XYChart.Series<>();
            serie.setName("Glicemia");

            List<Rilevazione> rilevazioniRecenti = paziente.getRilevazioni().stream()
            ...
            for (Rilevazione r : rilevazioniRecenti) {
                ...
                // Sostituisci getValore() con il tuo getter se ha un nome diverso (es. getGlicemia())
    ...
    private void modificaDatiPaziente(Paziente paziente) throws IOException {
        ...
        if (paziente.getBreveDescrizione() != null) {
        ...
        if (result.isPresent() && !result.get().isEmpty()) {
            ...
            paziente.setBreveDescrizione(descrizione);
            GestoreFile.salvaPaziente(paziente);

            String modifica = diabetologo.getCodiceFiscale() + " modifica note/descrizione paziente "
